...
trait GenericTrait
{
                ...
                try {
                    ...
                    if ($stockActual === null) {//si debo "anular" el stock, corresponde "anular" todos los restos del producto
                        ...
                        if ($tiene_receta) {//en este caso además debo agregar el stock que estoy anulando a sus ingredientes
                            $stockManualStatus = $this->agregarManualStockIngrediente($productoId, $diferenciaDeStock);
                        }
                       ...
                       // dd('stock cero');
                        ...
                        //en este caso debo agregar a sus ingredientes el stock que estoy anulando 
                        if ($tiene_receta) $stockManualStatus = $this->agregarManualStockIngrediente($productoId, $diferenciaDeStock);

                    } else {  //sino, veré si estoy agregando o quitando stock
                        //dd($diferenciaDeStock);
                        if ($diferenciaDeStock > 0) { //si agrego stock 
                            $stockManualStatus = $this->agregarManualStockProducto($productoId, $diferenciaDeStock, $costoHistorico);
                            if ($tiene_receta) $stockManualStatus = $this->descontarManualStockIngrediente($productoId, $diferenciaDeStock, $costoHistorico);
                        } else {
                           //dd('kjk' . $diferenciaDeStock);                     //si descuento stock
                            $stockManualStatus = $this->descontarManualStockProducto($productoId, $diferenciaDeStock);
                            if ($tiene_receta) $stockManualStatus = $this->agregarManualStockIngrediente($productoId, $diferenciaDeStock);
                        }
                    }
    ...
    public function agregarManualStockProducto($productoId, $diferenciaDeStock, $costoHistorico)
    {
        DB::beginTransaction();
        try {
            $cantidad_a_agregar = $diferenciaDeStock;
            //primero se lo agrego a las compras y luego a la EI
            $record = Peps::where('producto_id', $productoId)
                ->where('comercio_id', $this->comercioId)
                ->where('mov_stock_id', 2)
    ...
    public function agregarManualStockIngrediente($productoId, $diferenciaDeStock)
    {
        DB::beginTransaction();
        try {
                ...
                if($principal->count() > 0){
                    foreach ($principal as $i) {
                            ...
                            if ($record->count() > 0) {                       
                                foreach ($record as $i) {
                                    ...
                                    if ($nuevo_resto <= $i->cantidad){            
                                        ...
                                        $diferenciaDeStock = $i->cantidad - $i->resto;
                                        $resto_a_grabar = $resto_a_grabar - $diferenciaDeStock;
                                        $i->update(['resto' => $i->cantidad]);
                                    }   
                                }
                            }
                        }
                        //finalmente se lo agrego a la EI   0.500
                        if ($resto_a_grabar > 0) {
                            $cantidadEI = 0;
                            $restoEI = 0;
                        ...
                        $peps = Peps::create([
                            ...
                            'resto'        => $cantidad_para_grabar_movimiento,
    ...
    public function descontarManualStockProducto($productoId, $diferenciaDeStock)
    {
        DB::beginTransaction();
        try {    
    ...
    public function descontarManualStockIngrediente($productoId, $diferenciaDeStock, $costoHistorico)
    {
        $diferenciaDeStockParaMovimiento = $diferenciaDeStock;       
        DB::beginTransaction();
        try {
                ...
                if($principal->count() > 0){
                    foreach ($principal as $i) {
                        ...
                        $record = Peps::where('producto_id', $i->producto_id)->where('mov_stock_id', 1)
                                ->where('comercio_id', $this->comercioId)->first();
                        if ($record) {
                            $resto_a_grabar = $record->resto - $cantidad_real;                    //   -1=1-2
                            if ($resto_a_grabar >= 0) $record->update([ 'resto' => $resto_a_grabar ]);
                            else $record->update([ 'resto' => 0 ]); 
                        } 
                        // else {
                        ...
                        //             //dd($record);
                        // if ($record) {
                        //     $nueva_cantidad = $record->cantidad + $cantidad_real;
                        //     // $nueva_cantidad = $record->cantidad + $i->cantidad;
                        ...
                        // } else {
                            $peps = Peps::create([
                                ...
                                'resto'           => $cantidad_para_grabar_movimiento * -1,
                                'prod_modif_id'   => $productoId,
                                'cant_prod_modif' => $diferenciaDeStockParaMovimiento,
                                'user_id'         => $this->userId,
                                'comercio_id'     => $this->comercioId
    ...
    public function verificarStockTrait($cantidad, $productoId, $tipo)
    {
