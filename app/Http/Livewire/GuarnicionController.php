...
class GuarnicionController extends Component
{
    ...
    public function render()
    {
        ...
        {
            $info = Guarnicion::where('descripcion', 'like', '%' .  $this->search . '%')
                    ->where('comercio_id', $this->comercioId) 
                    ->orderby('descripcion','desc')->get();
            return view('livewire.guarniciones.component', [
                'info' =>$info
            ]);
        }
        else {
           return view('livewire.guarniciones.component', [
            'info' => Guarnicion::orderBy('descripcion', 'asc')
    ...
    {
        $this->action = 2;
        $record = Guarnicion::findOrFail($id);
        $this->selected_id = $id;
    ...
    public function RecuperarRegistro($id)
    {
        DB::begintransaction();
        try{
            Guarnicion::onlyTrashed()->find($id)->restore();
    ...
    { 
        ...
        try{
            if($this->selected_id > 0) {
                $existe = Guarnicion::where('descripcion', $this->descripcion)
                ...
                if($existe->count() && $existe[0]->deleted_at != null) {
                    ...
                    session()->flash('info', 'La Guarnicion ya existe...');
                    $this->resetInput();
                    return;
                }
            }else {
            ...
            if($this->selected_id <= 0) {
                $category =  Guarnicion::create([
                    'descripcion' => ucwords($this->descripcion), 
                    'comercio_id' => $this->comercioId            
                ]);
            }else {   
                $record = Guarnicion::find($this->selected_id);
                $record->update([
                    'descripcion' => ucwords($this->descripcion)
                ]);
                $this->action = 1;              
            }
            if($this->selected_id) session()->flash('msg-ok', 'Guarnicion Actualizada');            
            else session()->flash('msg-ok', 'Guarnicion Creada');            
    ...
    public function destroy($id, $comentario)
    {
        if ($id) {
            DB::begintransaction();
            try{
                $salsa = Guarnicion::find($id)->delete();
                $audit = Auditoria::create([
                    'item_deleted_id' => $id,
                    'tabla' => 'Guarniciones',
                    'user_delete_id' => auth()->user()->id,
