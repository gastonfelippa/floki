...
class SectorComandaController extends Component
{
    ...
    public function render()
    {
        ...
        {
            $info = SectorComanda::where('descripcion', 'like', '%' .  $this->search . '%')
                    ->where('comercio_id', $this->comercioId) 
                    ->orderby('descripcion','desc')->get();
            return view('livewire.sector_comanda.component', [
                'info' =>$info
            ]);
        }
        else {
           return view('livewire.sector_comanda.component', [
            'info' => SectorComanda::orderBy('descripcion', 'asc')
    ...
    {
        $this->action = 2;
        $record = SectorComanda::findOrFail($id);
        $this->selected_id = $id;
    ...
    public function RecuperarRegistro($id)
    {
        DB::begintransaction();
        try{
            SectorComanda::onlyTrashed()->find($id)->restore();
    ...
    { 
        ...
        try{
            if($this->selected_id > 0) {
                $existe = SectorComanda::where('descripcion', $this->descripcion)
                ...
                if($existe->count() && $existe[0]->deleted_at != null) {
                    ...
                    session()->flash('info', 'El Sector ya existe...');
                    $this->resetInput();
                    return;
                }
            }else {
            ...
            if($this->selected_id <= 0) {
                $category =  SectorComanda::create([
                    'descripcion' => ucwords($this->descripcion), 
                    'comercio_id' => $this->comercioId            
                ]);
            }else {   
                $record = SectorComanda::find($this->selected_id);
                $record->update([
                    'descripcion' => ucwords($this->descripcion)
                ]);
                $this->action = 1;              
            }
            if($this->selected_id) session()->flash('msg-ok', 'Sector Actualizado');            
            else session()->flash('msg-ok', 'Sector Creado');            
    ...
    public function destroy($id, $comentario)
    {
        if ($id) {
            DB::begintransaction();
            try{
                $sector_comanda = SectorComanda::find($id)->delete();
                $audit = Auditoria::create([
                    'item_deleted_id' => $id,
                    'tabla' => 'SectorComanda',
                    'user_delete_id' => auth()->user()->id,
