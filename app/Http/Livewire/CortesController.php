...
class CortesController extends Component
{
    ...
    public $cajaInicial, $ventas, $cobrosCtaCte, $otrosIngresos, $egresos, $cajaFinal, $Arqueo;
    public $comercioId, $selected_id = null, $importe, $nro_caja = 1, $comentario='', $caja_abierta;
    ...
    public function render()
    {
        ...
        if($this->user == 0){
            $caja_abierta = CajaUsuario::where('caja_usuarios.caja_usuario_id', auth()->user()->id)
                ...
                ->orWhere('caja_usuarios.user_id', auth()->user()->id)
                ->where('caja_usuarios.estado', '1')
                ->select('caja_usuarios.*')->get();           
        ...
        $factPendiente = Factura::where('estado', 'pendiente')
            ->where('repartidor_id', $this->user)->get();
        if($factPendiente->count() > 0) $this->factPendiente = 1;
    ...
    { 
        ...
        if($this->user == 0)
        {
            $this->cajaInicial = CajaUsuario::join('cajas as c', 'c.id', 'caja_usuarios.caja_id')
                ...
                ->where('c.comercio_id', $this->comercioId)->sum('importe');
